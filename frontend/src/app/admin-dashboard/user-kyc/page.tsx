...
export default function AdminUserKycPage() {
  ...
  const [previewData, setPreviewData] = useState<{url: string, user: AdminUserKyc, type: 'dl' | 'aadhaar'} | null>(null);
  const LIMIT = 15;
                  ...
                  <tr key={u.id} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-4 py-4 hidden md:table-cell">
                      <div className="flex flex-col gap-2">
                        {u.user_dl_url ? (
                          <button onClick={() => setPreviewData({url: `${API_BASE}${u.user_dl_url}`, user: u, type: 'dl'})} className="flex items-center gap-2 text-[11px] font-bold text-slate-300 hover:text-white transition group">
                        ...
                        {u.user_aadhaar_url ? (
                          <button onClick={() => setPreviewData({url: `${API_BASE}${u.user_aadhaar_url}`, user: u, type: 'aadhaar'})} className="flex items-center gap-2 text-[11px] font-bold text-slate-300 hover:text-white transition group">
